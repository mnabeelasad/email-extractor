...
#     import os
#     own_address = os.environ.get("EMAIL_USER", "").lower()
...
#         # 0. skip our own outgoing notification emails (sender == this mailbox)
...
def main(source: str = "real"):
    ...
    for msg in messages:
        ...
        # 1. which client does this sender belong to?
        #    (scan subject + body too, so forwarded/mentioned client emails route correctly)
        raw_body = msg.get("body", {}).get("content", "")
        scan_text = f"{subject}\n{raw_body}"
        client_folder = get_client_folder(sender, scan_text)
        client_key = str(client_folder)
